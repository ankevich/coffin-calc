...
const getLifeExpectancy = (countryName, year) =>
    expectationsData // Берем массив с нашими данными
        ...
        .map(parseFloat)[0] // Преобразуем год из строки во Float // Вытаскиваем из массива первый результат
...
function App() {
    ...
    return (
        <div className="App">
            ...
            <p>
                Привет, друг! Наверняка ты задумывался о том, как близок час
                твоей кончины. Ну так вот, теперь у тебя пояивилась уникальная
                возможность узнать об этом. Будь осторожен, тебе потом жить с этой инфой.
            </p>
            <h3>Имя</h3>
            ...
            <p>
                <button onClick={() => setAnswerHidden(false)}>
                    Посчитать
                </button>
            </p>
            <p hidden={hideAnswer}>
                Привет{name === '' ? ',' : `, ${name}, `}{' '}
                {remainingAge > 0
                    ? `тебе осталось жить примерно ${remainingAge} лет.`
                    : `твое место уже в ⚰️, узбагойся!`}
                <br />
                {remainingAge > 0
                    ? `Ты не умрёшь раньше ${lifeExpectancy}.`
                    : `Тебя не стало в ${lifeExpectancy} лет.`}
            </p>
        </div>
    )
}
